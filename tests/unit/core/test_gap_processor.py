...
def test_extract_features_from_batch():
    """Verify extract_features resolves columns from batch."""
    proc = GapProcessor(name="test", config={"columns": {"input": ["a", "b"]}})
    batch = pa.RecordBatch.from_arrays([pa.array([1]), pa.array([2])], names=["a", "b"])
    features = proc.extract_features(batch, {})
    assert "a" in features
    assert "b" in features


def test_extract_features_from_prev_features():
    """Verify extract_features skips columns in prev_features (resolved from there)."""
    proc = GapProcessor(name="test", config={"columns": {"input": ["a", "b"]}})
    batch = pa.RecordBatch.from_arrays([pa.array([1])], names=["a"])
    prev = {"b": pa.array([10])}
    features = proc.extract_features(batch, prev)
    assert "a" in features
...
def test_extract_features_no_input_config():
...
def test_extract_features_column_missing_logs_warning(caplog):
