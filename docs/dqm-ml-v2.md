...
The orchestration is handled by the `DatasetPipeline` class in `dqm-ml-job`. It manages the lifecycle of the processors and ensures that only the required data columns are loaded from the source, further optimizing I/O performance.
