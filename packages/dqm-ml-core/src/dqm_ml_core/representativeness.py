...
class RepresentativenessProcessor(DatametricProcessor):
    """
    TODO: this metric is compute for juste one colomn : only performs a one-sample test — it compares your data to a target distribution (normal or uniform).
    ...
    def __init__(self, name: str = "representativeness", config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.shannon_entropy_threshold: float = float(cfg.get("shannon_entropy_threshold", self.DEFAULT_SHANNON_ENTROPY_THRESHOLD))
        ...
        self.interpretation_thresholds: Dict[str, str] = cfg.get("interpretation_thresholds", self.DEFAULT_INTERPRETATION_THRESHOLDS)
    ...
    def compute_batch_metric(self, features: Dict[str, pa.Array]) -> Dict[str, pa.Array]:
        ...
        for col in self.columns:
            ...
            batch_metrics[f"{col}_hist"] = pa.FixedSizeListArray.from_arrays(hist_counts, list_size=hist_counts.shape[0])
            ...
            if "kolmogorov-smirnov" in self.metrics:
                ...
                sample_per_batch = min(self.ks_sample_size, max(self.ks_min_sample_size, len(values) // self.ks_sample_divisor)) # ex echantilloange
    ...
    def compute(self, batch_metrics: Optional[Dict[str, pa.Array]] = None) -> Dict[str, Any]:
        ...
        for col in self.columns:
            ...
            for batch_hist in hist_batch_arrays:
                if hist_arrays is None:
                    ...
                    hist_arrays = hist_arrays + batch_hist
                    ...
                    if exp_sum > 0:
                        ...
                        try:
                            ...
                            col_res["chi-square"] = {
                                "p_value": float("nan"), 
                                "statistic": float("nan"), 
                                "interpretation": f"chi_square_failed: {str(e)}",
                                "note": "using observed counts only due to statistical constraints"
                            }
                    else:
                        col_res["chi-square"] = {"p_value": float("nan"), "statistic": float("nan"), "interpretation": "no_expected_counts"}
                else:
                    col_res["chi-square"] = {"p_value": float("nan"), "statistic": float("nan"), "interpretation": "insufficient_bins"}
            ...
            if "kolmogorov-smirnov" in self.metrics:
                ...
                if sample_key in batch_metrics:
                    sample_arrays = batch_metrics[sample_key].to_numpy()
                    if sample_arrays.ndim == 1:
                        ks_samples = sample_arrays
                    else:
                        ks_samples = sample_arrays.flatten()
