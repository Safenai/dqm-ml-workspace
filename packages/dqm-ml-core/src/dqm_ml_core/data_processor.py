    ...
    def __init__(self, name, config: Optional[dict[str, Any]] = None):
        ...
        if 'input_columns' in self.config:
            if not isinstance(self.config['input_columns'], list):
                raise ValueError(f"Metric {name} configuration must contain a list of 'input_columns', got {type(self.config['input_columns'])}")
        ...
        if 'output_columns' in self.config:
            if not isinstance(self.config['output_columns'], dict):
                raise ValueError(f"Metric {name} configuration must contain a dictionary of 'output_columns', got {type(self.config['output_columns'])}")
